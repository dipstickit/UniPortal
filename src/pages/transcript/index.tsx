...
const Ranking = () => {
  ...
  const form = useForm<z.infer<typeof updateForm>>({
    ...
    defaultValues: {
      id:
        data?.data?.studentRecords.forEach((record) => record.id) || undefined,
      subjectId:
        data?.data?.studentRecords.forEach((record) => record.subject.id) ||
        undefined,
      mark:
        data?.data?.studentRecords.forEach((record) => record.mark) ||
        undefined,
    },
  });
  ...
  return (
    <div className="">
      <Form {...form}>
        <form
          className="flex flex-col items-center text-black justify-center p-4 shadow-md bg-slate-50/50 w-full"
        ...
        >
          ...
          <div className="text-center mt-8 w-1/2 ">
            <Button
              type="submit"
              className="w-full text-xl p-4 focus:ring-2 focus:ring-offset-2 focus:ring-black/50"
            >
              Cập nhật
            </Button>
          </div>
